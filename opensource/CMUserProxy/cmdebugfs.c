...
void cm_debugfs_init(void)
{
	if ((is_debugfs())
		...
		dir = opendir(path);
		if (dir == NULL) {
			ALOGI("Meticulus: %s not found in debugfs\n", DEBUGFS_ROOT);
			debugfs_path = NULL;
		} else {
			ALOGI("Meticulus: %s found in debugfs, mounted on %s\n",
			     DEBUGFS_ROOT, path);
			closedir(dir);
...
void copy(char *from_file, char *to_file, char* base, char* prefix, char* dst)
{
	...
	// Meticulus:
	// Bionic says we can't include O_CREAT  withou including mode_t
	// Pick 0660 out of the air.
	in = open(from, O_RDONLY);
	out = open(to, O_WRONLY | O_CREAT, 0660);
